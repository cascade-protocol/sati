...
- Hardened adapters with optional chaining and validation guards for malformed on-chain data
...
- **`MAX_SINGLE_SIGNATURE_CONTENT_SIZE` re-export replaced** with `MAX_COUNTERPARTY_SIGNED_CONTENT_SIZE` (100 bytes) - matches sati-sdk 0.9.0

### Dependencies

- Requires `@cascade-fyi/sati-sdk` >= 0.9.0

## [0.5.0] - 2026-02-12

### Breaking Changes

- **Feedback fields aligned with ERC-8004**: `score` -> `value`, `tags` -> `tag1`/`tag2`, added `valueDecimals` - matches sati-sdk 0.8.0 changes
- **`getReputationSummary` signature changed**: `(agentId, tags?)` -> `(agentId, tag1?, tag2?)`
- **`ReputationSummary.averageScore` renamed** to `averageValue`
- **Registration file fields renamed**: `endpoints` -> `services`, `x402support` -> `x402Support`

### Dependencies

- Requires `@cascade-fyi/sati-sdk` >= 0.8.0
